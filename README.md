...
## V0.5.9 — Preenchimento assistido iMile

O Hub prepara a iMile com relação mapeada, nome, número de documento e notas,
somente após ativação explícita do serviço Android. O entregador continua
responsável por selecionar documento não-CPF, registrar foto, colher assinatura
e tocar em **Entregue** no aplicativo da iMile.

Para entregas iMile, o Hub agora exige foto de comprovante de recebimento, quem recebeu, nome completo, tipo/número de documento e assinatura antes de confirmar o POD local. A baixa no app iMile continua manual nesta versão; a automação será investigada separadamente.

## V0.5.7 — Scanner manual pela câmera e atualização resiliente
...
- Uma etiqueta com código válido, mas sem padrão de empresa, agora abre pela
  **câmera** a seleção manual de transportadora após duas leituras do mesmo código.
- A checagem de atualização não depende mais de `navigator.onLine`, que pode ser
  incorreto no WebView Android, e usa `/app-update.json` como rota de reserva.
...
O roteiro completo está em `docs/V0.5.7_SCANNER_DESCONHECIDO_E_ATUALIZACAO.md`.
