...
function extrairTrackingDeTextoScanner(texto: string): string | null {
  ...
  const anjunDentroDoTexto = texto.match(/AJ\d{14,15}/i);
  return anjunDentroDoTexto?.[0]?.trim() ?? null;
}

function limparTexto(texto: string): string {
  // Etiquetas iMile podem imprimir o tracking em grupos (ex.: 60905264-49890).
  // O scanner de barras normalmente remove o separador, mas OCR e digitação não.
  return texto.trim().replace(/^`+/, "").replace(/[\s-]+/g, "").toUpperCase();
}
