...
import { criarReferenciaCarga, criarReferenciaLote } from "./referencias-carga";
...
export function criarCargaManual(
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(dataOperacao) || Number.isNaN(Date.parse(`${dataOperacao}T00:00:00`))) {
    throw new Error("Informe uma data de operacao valida.");
  }
  ...
  return {
    ...
    referenciaCarga: criarReferenciaCarga(cargaId, dataOperacao),
    referenciaLote: criarReferenciaLote(`MANUAL-${cargaId}`, dataOperacao),
    sequenciaNoLote: 1,
...
export function transferirPacote(
  ...
  if (origem.dataOperacao !== destino.dataOperacao) {
    throw new Error("A transferencia deve permanecer na mesma data de operacao.");
  }
  if ((origem.status ?? "PUBLICADA") === "ENCERRADA" || (destino.status ?? "PUBLICADA") === "ENCERRADA") {
    throw new Error("Nao e possivel transferir pacotes de ou para uma carga encerrada.");
  }
...
export function publicarCarga(carga: CargaEntregador): void {
  if (carga.status === "ENCERRADA") {
    throw new Error("Uma carga encerrada nao pode ser publicada novamente.");
  }
  if (carga.status === "EM_OPERACAO") {
    throw new Error("Uma carga em operacao nao pode voltar ao estado publicada.");
  }
...
export function encerrarCarga(carga: CargaEntregador): void {
  ...
  carga.motivoEncerramento = "CONCLUIDA";
}

export function podeExcluirCargaDeTeste(carga: CargaEntregador): boolean {
  return carga.pacotes.every((pacote) => {
    const estado = obterEstadoEntrega(pacote);
    return estado.estadoFisico === "PENDENTE" && estado.estadoIntegracao === "NAO_INICIADA";
  });
}

/**
 * Remove a carga da operacao sem apagar o registro remoto. O tombstone chega
 * aos aparelhos e evita que um cache offline ressuscite uma carga excluida.
 */
export function excluirCargaDeTeste(carga: CargaEntregador): void {
  if (!podeExcluirCargaDeTeste(carga)) {
    throw new Error("A carga possui operacao iniciada e nao pode ser removida como teste.");
  }
  carga.pacotes = [];
  carga.status = "ENCERRADA";
  carga.encerradaEm = agora();
  carga.motivoEncerramento = "EXCLUIDA_TESTE";
}
