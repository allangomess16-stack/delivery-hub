...
export function particionarCargaPorPerfil(
  ...
  const perfisComPacotes = [...perfisUnicos.values()].filter((perfil) =>
    (pacotesPorEntregador.get(perfil.entregadorId) ?? []).length > 0
  );

  return perfisComPacotes.map((perfil, indice) => {
    ...
    return {
      ...
      referenciaLote: criarReferenciaLote(carga.id, dataOperacao),
      sequenciaNoLote: indice + 1,
      ...
      lotesOrigem: [criarOrigemLote(carga, dataOperacao, pacotes.length)],
    };
  });
}
