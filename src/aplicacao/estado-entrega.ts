...
function converterRecebedorImile(tipo: TipoRecebedorImile): RecebedorEntrega["tipo"] {
  if (tipo === "PROPRIO" || tipo === "PORTARIA" || tipo === "FAMILIAR" || tipo === "VIZINHO") return tipo;
  return "OUTRO";
}

export function definirDadosComprovacaoImile(
  pacote: PacoteDaCarga,
  dados: DadosComprovacaoImile,
): void {
  const nomeCompleto = dados.nomeCompleto.trim();
  const numeroDocumento = dados.numeroDocumento.trim();
  if (!nomeCompleto || nomeCompleto.length > 160) {
    throw new Error("Informe o nome completo de quem recebeu.");
  }
  if (!numeroDocumento || numeroDocumento.length > 80) {
    throw new Error("Informe o numero do documento para a iMile.");
  }
  if (dados.observacao && dados.observacao.length > 500) {
    throw new Error("A observacao pode ter no maximo 500 caracteres.");
  }

  const entrega = obterEstadoEntrega(pacote);
  const normalizado: DadosComprovacaoImile = {
  ...
  entrega.dadosComprovacaoImile = normalizado;
  definirRecebedor(pacote, {
    tipo: converterRecebedorImile(normalizado.recebedor),
    nome: normalizado.nomeCompleto,
    documento: normalizado.numeroDocumento,
  });
}

export function validarProvaImile(pacote: PacoteDaCarga): void {
  const entrega = obterEstadoEntrega(pacote);
  const dados = entrega.dadosComprovacaoImile;
  if (!dados) throw new Error("Preencha os dados de recebimento exigidos pela iMile.");
  if (!entrega.fotos.some((foto) => foto.tipo === "COMPROVANTE_RECEBIMENTO")) {
    throw new Error("Registre o comprovante de recebimento para a iMile.");
  }
  if (!entrega.assinatura) throw new Error("Registre a assinatura do cliente para a iMile.");
}
...
export function confirmarEntrega(pacote: PacoteDaCarga): void {
  ...
  if (pacote.transportadora.id === "IMILE") validarProvaImile(pacote);
