...
import type { MotivoNaoEntrega, OrigemEvidenciaFoto, TipoEvidenciaFoto, TipoRecebedor } from "../../dominio/entrega/tipos";
import {
  adicionarFoto,
  cancelarPreparacao,
  definirAssinatura,
...
type TelaEntrega = "FOTOS" | "RECEBEDOR" | "ASSINATURA" | "FINALIZAR";
...
export class ControladorFluxoEntrega {
  ...
  private mostrarResultado(): void {
    ...
    const abrirTransportadora = async () => {
      ...
      if (status) status.textContent = "POD salvo. Abrindo aplicativo da transportadora...";
      try {
