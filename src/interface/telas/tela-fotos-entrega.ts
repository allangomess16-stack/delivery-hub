...
export function telaFotosEntrega(pacote: PacoteDaCarga) {
  const exigeComprovanteImile = pacote.transportadora.id === "IMILE";
  return `
    ${cabecalhoFixo(
    ...
    <main class="conteudo conteudo--com-rodape">
      <section class="cabecalho-etapa">
        <span class="sobrelinha">ETAPA 1${exigeComprovanteImile ? " • IMILE" : ""}</span>
        <h1>Fotos de comprovação</h1>
        <p>
          ${exigeComprovanteImile
            ? "A iMile exige uma foto nítida do recebimento. Use a câmera do Hub ou anexe uma foto já feita pela galeria."
            : "Use a câmera do Hub ou fotografe antes no celular e anexe depois pela galeria. As fotos ficam guardadas neste aparelho, mesmo sem rede."}
        </p>
      </section>
      ...
      <section class="grade-fotos">
        ...
        ${exigeComprovanteImile
          ? fotoCard(
            ...
            "Foto nítida exigida para concluir a prova da iMile.",
          )
          : ""}
      </section>
    </main>
