...
function card(registro: RegistroIntegracaoTransportadora): string {
  const principais = registro.capacidades
    .filter((item) =>
      ["ABRIR_APP", "DEEP_LINK", "COMPARTILHAR_TEXTO", "PROVA_FOTO", "MODO_OFFLINE"].includes(
        item.capacidade,
      ),
    )
    .map(
  ...
  return `
    ...
    <main class="conteudo conteudo--com-rodape">
      <section class="painel-destaque">
        ...
        <p>
          Cada aplicativo possui contrato e fallback separados por versionCode.
          Descoberta de codigo nunca promove automaticamente outra versao.
        </p>
      </section>
