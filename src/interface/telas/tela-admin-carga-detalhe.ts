...
import { listarOrigensLote, obterReferenciaCarga } from "../../aplicacao/carga/referencias-carga";
import { podeExcluirCargaDeTeste } from "../../aplicacao/carga/gestao-manual-carga";
...
export function htmlPacoteAdminCard(carga: CargaEntregador, pacote: PacoteDaCarga, perfis: PerfilEntregador[]): string {
  ...
  const extraRotaAberta = pacote.alertaAdmin && pacote.conciliacaoExtraRota?.status !== "RESOLVIDA";
    ...
    <article class="pacote-admin-card" data-pacote-id="${escaparHtml(pacote.id)}" data-pacote-codigo="${escaparHtml(pacote.codigoNormalizado)}" data-pacote-regiao="${escaparHtml(regiaoId)}">
      <div class="pacote-admin-card__topo">
        ...
        <div class="pacote-admin-card__conteudo">
          ...
          ${extraRotaAberta ? `<b class="alerta-extra-rota-label">EXTRA ROTA · CONCILIAÇÃO PENDENTE</b>` : ""}
      ...
      <div class="pacote-admin-card__acoes pacote-admin-card__acoes--regiao">
        ...
        ${extraRotaAberta ? `<button class="botao-mini botao-mini--atencao" data-resolver-extra-rota="${escaparHtml(pacote.id)}">RESOLVER ALERTA</button>` : ""}
        ${alteravel ? `
  ...
  return `
    ...
    <main class="conteudo conteudo--com-rodape" data-tela="admin-carga-detalhe">
      <section class="cabecalho-etapa">
        <span class="sobrelinha">${escaparHtml(referencia)} • ${escaparHtml(status)}</span>
        <h1><span id="total-pacotes-carga">${carga.pacotes.length}</span> pacotes</h1>
        <p>${escaparHtml(carga.nomeEntregador)} • ${escaparHtml(carga.dataOperacao)}</p>
      </section>

      <section class="carga-origens-detalhe">
        ${origens.map((origem) => `
          <article>
            <strong>${escaparHtml(origem.referencia)}</strong>
            <span>${escaparHtml(origem.nomeArquivo)}</span>
            <b>${origem.quantidadePacotes} pacotes</b>
          </article>
        `).join("")}
      </section>
      ...
      ${status !== "ENCERRADA" && podeExcluirCargaDeTeste(carga) ? `
        <section class="painel-remocao-carga">
          <span class="sobrelinha">CONTROLE DA CARGA</span>
          <strong>Remover da operacao</strong>
          <button id="excluir-carga-teste" class="botao-acao botao-acao--perigo">REMOVER CARGA DE TESTE</button>
        </section>
      ` : ""}
    </main>
