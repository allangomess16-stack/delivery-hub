    ...
    <main class="conteudo conteudo--com-rodape">
      ...
      ${duplicados.length ? `
        <section class="secao-lista">
          <div class="titulo-secao"><div><span class="sobrelinha">IMPORTACAO BLOQUEADA</span><h2>Onde estao os duplicados</h2></div><span>${duplicados.length}</span></div>
          <div class="lista-revisao">
            ${duplicados.map((item) => `<article class="revisao-item"><strong>${escaparHtml(item.codigo)}</strong><span>${escaparHtml(descreverDuplicidade(item).replace(`Tracking ${item.codigo}: `, ""))}</span></article>`).join("")}
          </div>
        </section>` : ""}
