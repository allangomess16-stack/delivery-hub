...
export function telaAssinaturaEntrega(pacote: PacoteDaCarga): string {
  ...
  const exigeAssinaturaImile = pacote.transportadora.id === "IMILE";
  return `
    ${cabecalhoFixo(
    ...
    <main class="conteudo conteudo--com-rodape">
      <section class="cabecalho-etapa">
        <span class="sobrelinha">ETAPA 3${exigeAssinaturaImile ? " • IMILE" : ""}</span>
        <h1>Assinatura</h1>
        <p>${exigeAssinaturaImile
          ? "A assinatura do cliente é obrigatória para a prova da iMile. Ela permanece protegida no aparelho até a sincronização."
          : "Registre somente quando a operacao exigir. Ela permanece protegida no aparelho ate a sincronizacao."}</p>
      </section>
    ...
    <footer class="acoes-fixas">
      ...
      <button id="ir-finalizar" class="botao-acao botao-acao--primario" ${exigeAssinaturaImile && !registrada ? "disabled" : ""}>${registrada ? "PRÓXIMO" : exigeAssinaturaImile ? "ASSINATURA OBRIGATÓRIA" : "CONTINUAR SEM ASSINATURA"}</button>
    </footer>
  `;
}
