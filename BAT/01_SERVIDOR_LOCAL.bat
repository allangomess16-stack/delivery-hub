...
if errorlevel 1 (
  ...
  choice /C SN /N /M "Liberar acesso do celular na rede privada agora? [S/N]: "
  if not errorlevel 2 (
    echo.
    echo Solicitando permissao do Windows...
    powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ^
      "Start-Process -FilePath 'cmd.exe' -Verb RunAs -Wait -ArgumentList '/c ""%~dp004_CORRIGIR_ACESSO_CELULAR.bat"" /firewall'"
    if errorlevel 1 (
      echo [ATENCAO] A regra nao foi criada. O PC continuara funcionando;
      echo           para o celular, execute BAT\04_CORRIGIR_ACESSO_CELULAR.bat.
    ) else (
      echo [OK] Regra privada da porta 5173 criada.
    )
  ) else (
    echo [INFO] Firewall mantido sem alteracao. O PC abrira normalmente.
  )
) else (
...
echo.
echo O navegador do PC sera aberto automaticamente.
echo No celular, abra o endereco LOCAL exibido abaixo, usando a mesma rede Wi-Fi.
echo Mantenha esta janela aberta enquanto PC e celular estiverem usando o sistema.
echo.
