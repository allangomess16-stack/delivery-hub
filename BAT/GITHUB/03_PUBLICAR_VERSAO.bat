...
if not exist ".git" (
  echo [INFO] Git local ainda nao configurado nesta pasta.
  echo [INFO] Executando a configuracao inicial automaticamente...
  call "%RAIZ%\BAT\GITHUB\01_CONFIGURAR_REPOSITORIO.bat"
  if errorlevel 1 (
    popd
    echo [ERRO] A configuracao inicial do Git nao foi concluida.
    pause
    exit /b 1
  )
)
