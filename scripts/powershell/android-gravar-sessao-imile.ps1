...
# No PowerShell 7, mensagens de progresso escritas pelo adb em stderr podem
# virar excecoes mesmo quando o comando terminou com codigo 0. O ADB continua
# sendo validado por $LASTEXITCODE e pela existencia do arquivo copiado.
...
$InternalDirectory = Join-Path $OutputDirectory "interno"
$LogProcess = $null
$VideoProcess = $null
...
function Invoke-AdbCaptured {
  ...
  # Start-Process separa stdout/stderr: avisos da MIUI deixam de ser excecao
  # fatal no PowerShell, inclusive quando o ADB termina com codigo zero.
  $stdoutFile = Join-Path $env:TEMP ("deliveryhub-adb-$([guid]::NewGuid().ToString('N')).out")
  $stderrFile = Join-Path $env:TEMP ("deliveryhub-adb-$([guid]::NewGuid().ToString('N')).err")
  try {
    ...
    $output = if (Test-Path -LiteralPath $stdoutFile) { [string](Get-Content -LiteralPath $stdoutFile -Raw -ErrorAction SilentlyContinue) } else { "" }
    $stderr = if (Test-Path -LiteralPath $stderrFile) { [string](Get-Content -LiteralPath $stderrFile -Raw -ErrorAction SilentlyContinue) } else { "" }
    return [pscustomobject]@{ ExitCode = $process.ExitCode; Output = $output.Trim(); Error = $stderr.Trim() }
  } finally {
...
function Invoke-AdbText {
  ...
  $result = Invoke-AdbCaptured @Arguments
  $text = [string]$result.Output
  if ($result.ExitCode -ne 0 -and -not [string]::IsNullOrWhiteSpace($result.Error)) {
    $text = "$text`r`n$($result.Error)"
  }
  return (Protect-SensitiveText ([string]$text).Trim())
}
...
function Save-UiHierarchy {
  ...
  $remoteUi = "/sdcard/deliveryhub-imile-ui-$Timestamp-$Stage.xml"
  $localUi = Join-Path $InternalDirectory "$Stage-ui.xml"
  $dump = Invoke-AdbCaptured shell uiautomator dump $remoteUi
  if ($dump.ExitCode -ne 0) {
    "uiautomator dump indisponivel (exit code $($dump.ExitCode)): $($dump.Output) $($dump.Error)" | Set-Content -LiteralPath $localUi -Encoding UTF8
    return
  }
  $pull = Invoke-AdbCaptured pull $remoteUi $localUi
  if ($pull.ExitCode -ne 0 -or -not (Test-Path -LiteralPath $localUi)) {
    "Nao foi possivel copiar a hierarquia UI (exit code $($pull.ExitCode)): $($pull.Output) $($pull.Error)" | Set-Content -LiteralPath $localUi -Encoding UTF8
    return
  }
  Protect-SensitiveText ([string](Get-Content -LiteralPath $localUi -Raw -ErrorAction SilentlyContinue)) | Set-Content -LiteralPath $localUi -Encoding UTF8
}

function Save-InternalSnapshot {
  param([string]$Stage, [string]$Description)

  $activity = Invoke-AdbText shell dumpsys activity top
  $intents = Invoke-AdbText shell dumpsys activity intents
  $package = Invoke-AdbText shell dumpsys package com.imile.redelivery
  $activityRelevant = $activity -split "`r?`n" | Where-Object { $_ -match 'ACTIVITY|mResumedActivity|mFocusedApp|topResumedActivity|Intent|com\.imile\.redelivery|com\.deliveryhub\.(app|homologacao)' } | Select-Object -First 300
  $intentRelevant = $intents -split "`r?`n" | Where-Object { $_ -match 'com\.imile\.redelivery|com\.deliveryhub\.(app|homologacao)|action=|data=|extras=|cmp=' } | Select-Object -First 300
  $packageRelevant = $package -split "`r?`n" | Where-Object { $_ -match 'Activity|Service|Receiver|Permission|intent-filter|android\.intent\.action|com\.imile\.redelivery' } | Select-Object -First 500
  @(
    "DELIVERY HUB - COLETA INTERNA IMILE", "Descricao: $Description", "Capturado em: $(Get-Date -Format 'yyyy-MM-dd HH:mm:ss zzz')", "",
    "ACTIVITY NO TOPO", ($activityRelevant -join "`r`n"), "", "HISTORICO DE INTENTS FILTRADO", ($intentRelevant -join "`r`n"), "",
    "COMPONENTES E PERMISSOES DO PACOTE", ($packageRelevant -join "`r`n")
  ) -join "`r`n" | ForEach-Object { Protect-SensitiveText $_ } | Set-Content -LiteralPath (Join-Path $InternalDirectory "$Stage-android.txt") -Encoding UTF8
  Save-UiHierarchy $Stage
}

function Read-InternalMarker {
  param([string]$Stage, [string]$Instruction)
  Write-Host ""; Write-Host "MARCADOR $Stage"; Write-Host $Instruction
  Write-Host "Pressione ENTER para coletar Activity, intents e hierarquia interna agora."
  Read-Host | Out-Null
  Save-InternalSnapshot $Stage $Instruction
  Write-Host "[OK] Coleta interna salva: $Stage"
}
...
try {
  ...
  Save-InternalSnapshot "00-inicial" "Tela antes de iniciar a navegacao."
  Invoke-AdbCaptured logcat -c | Out-Null
  ...
  Write-Host ""
  Write-Host "Quando chegar a cada tela importante, volte ao computador para confirmar o marcador."
  Read-InternalMarker "01-apos-scan" "Deixe a primeira tela da iMile aberta apos o tracking ser reconhecido."
  Read-InternalMarker "02-formulario-recebimento" "Avance ate o formulario Quem recebeu, sem confirmar a baixa."
  Read-InternalMarker "03-formulario-final" "Avance ate a tela final com foto, documento ou assinatura, sem confirmar."
  Write-Host "Continue a navegacao ate concluir o roteiro seguro e depois volte ao computador."
  Read-Host | Out-Null
  ...
  Save-InternalSnapshot "04-final" "Estado final apos retornar ao computador."

  Write-Host "[1/3] Copiando gravacao do celular..."
  $pullOutput = Invoke-AdbCaptured pull $RemoteVideo $LocalVideo
  $videoOk = $pullOutput.ExitCode -eq 0 -and (Test-Path -LiteralPath $LocalVideo)
  if (-not $videoOk) {
    throw "Nao foi possivel copiar a gravacao do celular. ADB exit code: $($pullOutput.ExitCode). $($pullOutput.Output) $($pullOutput.Error)"
  }
  if ($videoOk) {
    Invoke-AdbCaptured shell rm $RemoteVideo | Out-Null
  }
  ...
  @(
    ...
    "Coleta interna: pasta interno (Activities, intents filtrados, componentes, hierarquia UI)",
